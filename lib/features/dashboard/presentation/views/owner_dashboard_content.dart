...
class OwnerDashboardContent extends ConsumerWidget {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          statsAsync.when(
            ...
            data: (stats) => GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return StatCard(title: item.title, value: item.value, subValue: item.subValue, color: Color(item.color), icon: iconData);
              },
            ),
          ),
          ...
          dutiesAsync.when(loading: () => const SizedBox(), error: (e, s) => const SizedBox(), data: (duties) => Column(children: duties.map((d) => DutyListItem(data: d)).toList())),
          
          const SizedBox(height: 24),
          _buildSectionTitle("Sự kiện đang mở"),
          eventsAsync.when(loading: () => const SizedBox(), error: (e, s) => const SizedBox(), data: (events) => Column(children: events.map((e) => EventCardItem(data: e)).toList())),
          ...
          Row(
            ...
            children: [
              ...
              Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: Colors.red[50], borderRadius: BorderRadius.circular(20)), child: Text("3 người", style: GoogleFonts.roboto(color: Colors.red, fontSize: 12, fontWeight: FontWeight.bold))),
            ],
          ),
  ...
  Widget _buildSectionTitle(String title) {
    return Padding(padding: const EdgeInsets.only(bottom: 12), child: Text(title, style: GoogleFonts.roboto(fontWeight: FontWeight.bold, fontSize: 16)));
  }
}